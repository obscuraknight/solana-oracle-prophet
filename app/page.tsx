  ...
  const consultProphet = () => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (priceMatch) {
          setPricePrediction(priceMatch[0]);
        } else {
          setPricePrediction("The future is unclear...");
        }

        const lowerAnswer = answer.toLowerCase();
        const bullishWords = ['bull', 'up', 'rise', 'higher', 'moon', 'grow', 'strong', 'to the moon'];
        const bearishWords = ['bear', 'down', 'drop', 'crash', 'lower', 'decline', 'dump'];

        const isBull = bullishWords.some(word => lowerAnswer.includes(word));
        const isBear = bearishWords.some(word => lowerAnswer.includes(word));
       
        setIsBullish(isBull && !isBear);
      } catch (err) {
        setPricePrediction("???");
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start justify-center">
          ...
          {/* LEFT COLUMN: Prophet + Diagram underneath */}
          <div className="flex-shrink-0 flex flex-col items-center lg:items-start">
            ...
            {/* Smaller Diagram - Now under the Prophet */}
            <div className="w-[380px] lg:w-[420px]">
              <h2 className="text-xl font-bold text-center mb-4 text-purple-400">🔮 Psychology of a Market Cycle</h2>
              <img
          ...
          {/* RIGHT COLUMN: Input + Predictions */}
          <div className="flex-1 max-w-lg lg:max-w-md pt-6">
            ...
            <div className="space-y-4">
              <input
                ...
                placeholder="What will SOL be in May 2029?"
      ...
      <footer className="mt-24 py-16 border-t border-purple-500/20 bg-black/60 text-center text-xs text-gray-400 leading-relaxed">
        <div className="max-w-2xl mx-auto px-6">
          ...
          • <strong>Birdeye.so</strong> and <strong>DEX Screener</strong> — on-chain liquidity &amp; volume analytics<br />
